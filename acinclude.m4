...
then
	...
	ac_gmime_cflags=`${gmimeconfig} --cflags gmime-3.0 2>/dev/null`
	if test -z "$ac_gmime_cflags"
	then
		AC_MSG_RESULT([no])
		AC_MSG_ERROR([Unable to locate gmime 3.0 development files])
	else
	...
	ac_gmime_libs=`${gmimeconfig} --libs gmime-3.0 2>/dev/null`
	if test -z "$ac_gmime_libs"
	then
		AC_MSG_RESULT([no])
		AC_MSG_ERROR([Unable to locate gmime 3.0 libaries])
	else
