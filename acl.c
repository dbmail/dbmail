...
int acl_has_right(u64_t userid, u64_t mboxid, 
		  ACLRight_t right)
{
...
int acl_set_rights(u64_t userid, u64_t mboxid,
	const char *rightsstring)
{
	// set rights.
...
{
	switch (right_char) {
	case 'l': return ACL_RIGHT_LOOKUP; 
	case 'r': return ACL_RIGHT_READ;
	case 's': return ACL_RIGHT_SEEN;
	case 'w': return ACL_RIGHT_WRITE;
	case 'i': return ACL_RIGHT_INSERT;
	case 'p': return ACL_RIGHT_POST;
	case 'c': return ACL_RIGHT_CREATE;
	case 'd': return ACL_RIGHT_DELETE;
	case 'a': return ACL_RIGHT_ADMINISTER;
	default:
		trace(TRACE_ERROR, "%s,%s: error wrong acl character. This "
...
int acl_change_rights(u64_t userid, u64_t mboxid, const char *rightsstring,
		      int set)
{
	unsigned i;
	char rightchar;
...
int acl_replace_rights(u64_t userid, u64_t mboxid, const char *rightsstring)
{
	unsigned i;
	int set;
	...
	for (i = ACL_RIGHT_LOOKUP; i < ACL_RIGHT_NONE; i++) {
		...
		if (strchr(rightsstring, (int) acl_right_chars[i]))
			...
			set = 0;
		if (db_acl_set_right(userid, mboxid,
				     acl_right_strings[i], set) < 0) {
...
int acl_set_one_right(u64_t userid, u64_t mboxid, ACLRight_t right, int set) 
{
	return db_acl_set_right(userid, mboxid, acl_right_strings[right], set);
}
...
int acl_get_rightsstring_identifier(char *identifier,
				    u64_t mboxid,
				    char *rightsstring)
{
	u64_t userid;
