 ...
 * sieve.c
 *
 * Implements a subset of RFC 5804 Remotely Managing Sieve Scripts.
...
#define MAX_FAULTY_RESPONSES 3

/* allowed sieve commands */
...
static int sieve(ClientSession_T *session);
static int sieve_tokenizer(ClientSession_T *session, char *buffer);
...
static void send_greeting(ClientSession_T *session)
{
	...
	if (strlen(banner) > 0)
		...
		ci_write(session->ci, "\"IMPLEMENTATION\" \"DBMail sieved %s\"\r\n", DM_VERSION);
	ci_write(session->ci, "\"SASL\" \"PLAIN\"\r\n");
	ci_write(session->ci, "\"SIEVE\" \"%s\"\r\n", sieve_extensions);
	ci_write(session->ci, "\"STARTTLS\"\r\n");
	ci_write(session->ci, "OK\r\n");
}

static void sieve_handle_input(void *arg)
{
	int l = 0;
	...
	while (TRUE) {
		...
		if ((l = sieve_tokenizer(session, buffer))) {
			if (l == -3) {
...
int sieve_handle_connection(client_sock *c)
{
...
int sieve_error(ClientSession_T * session, const char *formatstring, ...)
{
	va_list ap, cp;
	char *s;
...
int sieve_tokenizer(ClientSession_T *session, char *buffer)
{
	int command_type = 0;
	...
	if (! session->command_type) {
		...
		for (command_type = SIEVE_LOUT; command_type < (SIEVE_END); command_type++) {
			if (strcasecmp(command, commands[command_type]) == 0)
				break;
		}

		if ((value == NULL) && !(command_type <= SIEVE_LIST) && (command_type < SIEVE_END))
			return sieve_error(session, "NO \"This command requires an argument.\"\r\n");
...
int sieve(ClientSession_T *session)
{
	/* returns values:
	 *  0 to quit
	 * -1 on failure
	...
	case SIEVE_STLS:
		TRACE(TRACE_DEBUG, "[%p] cmd [%d], state [%d] [STARTTLS]", session, session->command_type, session->parser_state);
		if (session->state == CLIENTSTATE_AUTHENTICATED) {
			ci_write(session->ci, "Already authenticatd\r\n");
			return 1;
		}
		if (! server_conf->ssl) {
			ci_write(session->ci, "NO TLS not available\r\n");
			return 1;
		}
		if (session->ci->sock->ssl_state) {
			ci_write(session->ci, "NO TLS already active\r\n");
			return 1;
		}
		ci_write(session->ci, "OK Begin TLS now\r\n");
		iret = ci_starttls(session->ci);

		TRACE(TRACE_DEBUG, "[%p] cmd [%d], state [%d] [Returning]", session, session->command_type, session->parser_state);
	...
	case SIEVE_AUTH:
		...
		if (strcasecmp(arg, "PLAIN") == 0) {
			...
			if (! p_list_next(L))
				return sieve_error(session, "NO \"Missing argument.\"\r\n");
			L = p_list_next(L);
			s = p_list_data(L);
			arg = (char *)p_string_str(s);
			...
			if (tmp64 == NULL)
				return sieve_error(session, "NO \"SASL decode error.\"\r\n");
			for (i = 0; tmp64[i] != NULL; i++)
				;
			TRACE(TRACE_DEBUG, "SASL arguments [%d]", i);
			if (i < 3)
				sieve_error(session, "NO \"Too few encoded SASL arguments.\"\r\n");
			...
			if (auth_validate(ci, (const char *)tmp64[1], (const char *)tmp64[2], &useridnr) == 1) {
				...
				g_strfreev(tmp64);
				return sieve_error(session, "NO \"Username or password incorrect.\"\r\n");
			}
			g_strfreev(tmp64);
		} else
			return sieve_error(session, "NO \"Authentication scheme not supported.\"\r\n");
	...
	case SIEVE_PUTS:
		...
		if (scriptlen >= UINT_MAX)
			return sieve_error(session, "NO \"Invalid script length.\"\r\n");

		if (dm_sievescript_quota_check(session->useridnr, scriptlen))
			return sieve_error(session, "NO \"Script exceeds available space.\"\r\n");
		...
		if (dm_sievescript_add(session->useridnr, "@!temp-script!@", script)) {
			...
			return sieve_error(session, "NO \"Error inserting script.\"\r\n");
		}
		...
		if (sort_result == NULL) {
			...
			dm_sievescript_delete(session->useridnr, "@!temp-script!@");
			return sieve_error(session, "NO \"Script error: %s.\"\r\n", sort_get_errormsg(sort_result));
		}
	...
	case SIEVE_GETS:
		...
		if (! strlen(scriptname))
			return sieve_error(session, "NO \"Script name required.\"\r\n");

		ret = dm_sievescript_getbyname(session->useridnr, scriptname, &script);
		if (script == NULL) {
			return sieve_error(session, "NO \"Script not found.\"\r\n");
		} else if (ret < 0) {
			g_free(script);
			return sieve_error(session, "NO \"Internal error.\"\r\n");
		} else {
	...
	case SIEVE_DELS:
		...
		if (! dm_sievescript_delete(session->useridnr, scriptname))
			return sieve_error(session, "NO \"Error deleting script.\"\r\n");
		else
	...
	default:
		return sieve_error(session, "NO \"What are you trying to say here?\"\r\n");
	}
