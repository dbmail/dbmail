...
int pipe_forward(FILE *instream, list *targets, char *header, databasemessageid)
{
	...
	list *descriptors; /* target streams */
	element *target;
	...
	element = list_getstart (targets);

	while (element != NULL)
	{
		sendmail_command = (char *)malloc(strlen((char *)element->data)+
				strlen(SENDMAIL)+1); /* +1 for extra space */
		trace (TRACE_DEBUG,"pipe_forward(): allocated memory for"
				" external command call");

		sprintf (sendmail_command, "%s %s",SENDMAIL, (char *)element->data);
		...
		if (descriptors.total_nodes>0)
		{
			...
			while (descriptor_temp != NULL)
			{
				...
				{
					trace (TRACE_ERROR_,"pipe_forward(): descriptor value NULL"
		...
		{
			trace (TRACE_NOTICE,"pipe_forward(): No descriptors in list"
					" nothing to send");
			return -1;
		}
	}
