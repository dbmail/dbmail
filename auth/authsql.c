...
int auth_user_exists(const char *username, u64_t *user_idnr)
{
  ...
  snprintf(__auth_query_data, AUTH_QUERY_SIZE, 
		  "SELECT user_idnr FROM users WHERE userid='%s'", username);
...
int auth_get_known_users(struct list *users)
{
  ...
  if (db_num_rows() > 0) {
	  for (i = 0; i < (unsigned) db_num_rows(); i++) {
		  ...
		  if (!list_nodeadd(users, query_result, strlen(query_result)+1)) {
...
int auth_check_user (const char *username, struct list *userids, int checks) 
{
  ...
  if (checks > MAX_CHECKS_DEPTH) {
      trace(TRACE_ERROR, "%s,%s: maximum checking depth reached, "
  ...
  trace (TRACE_DEBUG,"%s,%s: into checking loop", __FILE__, __FUNCTION__);
  ...
  if (num_rows > 0)
    {
		for (counter = 0; counter < num_rows; counter++) {
			...
			trace (TRACE_DEBUG,"%s,%s: checking user %s to %s",
					__FILE__, __FUNCTION__, username, query_result);
  ...
  if (num_rows > 0) {
      for (counter=0; counter < num_rows; counter++) {
		  ...
		  trace (TRACE_DEBUG,"%s,%s: checking user %s to %s",
			 __FILE__, __FUNCTION__, username, query_result);
		  occurences += 
		       auth_check_user_ext (query_result, userids, fwds, 1);
	  }
  }
  db_free_result();
  db_set_result_set(saveres);
		  ...
		  char *clientid, char *maxmail, u64_t *user_idnr)
{
  ...
  *user_idnr = __auth_insert_result ("user_idnr");
  ...
  if (__auth_query(__auth_query_data) == -1) {
      trace (TRACE_ERROR,"%s,%s: query failed for adding mailbox",
			  __FILE__, __FUNCTION__);
	  db_store_auth_result();
      return -1;
    }
...
int auth_change_username(u64_t user_idnr, const char *new_name)
{
  ...
  if (__auth_query(__auth_query_data) == -1) {
      trace(TRACE_ERROR,"%s,%s: could not change name for user [%llu]",
			  __FILE__, __FUNCTION__, user_idnr);
	  db_store_auth_result();
      return -1;
  }
		...
		const char *enctype)
{
  ...
  if (__auth_query(__auth_query_data) == -1) {
      trace(TRACE_ERROR,"%s,%s: could not change passwd for user [%llu]",
			  __FILE__, __FUNCTION__, user_idnr);
	  db_store_auth_result();
      return -1;
  }
...
int auth_validate (char *username, char *password, u64_t* user_idnr)
{
	...
	*user_idnr = 0;
	...
	if (!query_result || strcasecmp(query_result, "") == 0) {
	     trace (TRACE_DEBUG,"%s,%s: validating using cleartext passwords",
		    __FILE__, __FUNCTION__);
		...
		is_validated = (strcmp(query_result, password) == 0) ? 1 : 0;
	     ...
	     trace (TRACE_DEBUG,"%s,%s: validating using crypt() encryption",
		    __FILE__, __FUNCTION__);
	     query_result = db_get_result(0, 1);
	     is_validated = 
		  (strcmp( 
		       (const char *) crypt(password, query_result), /* Flawfinder: ignore */ 
		       query_result) == 0) ? 1 : 0;
	     ...
	     if ( strncmp(query_result, "$1$", 3) ) {
		  ...
		  is_validated = (strncmp( 
				       makemd5(password), query_result, 32) == 0) ? 1 : 0;
	     } else {
		  trace (TRACE_DEBUG,"%s, %s: validating using MD5 hash comparison",
			 __FILE__, __FUNCTION__);
	     ...
	     trace (TRACE_DEBUG,"%s,%s: validating using MD5 digest comparison",
		    __FILE__, __FUNCTION__);
	     query_result = db_get_result(0, 1);
	     is_validated =
		  (strncmp( makemd5(password), query_result, 32) == 0) ? 1 : 0;
	}
		...
		char *apop_stamp)
{
  ...
  snprintf(checkstring, strlen(apop_stamp)+strlen(query_result)+2, 
		  "%s%s", apop_stamp, query_result);

  md5_apop_we=makemd5(checkstring);
	
  trace(TRACE_DEBUG,"%s,%s: checkstring for md5 [%s] -> result [%s]",
		  __FILE__, __FUNCTION__, checkstring, md5_apop_we);
  trace(TRACE_DEBUG,"%s,%s: validating md5_apop_we=[%s] md5_apop_he=[%s]",
		  __FILE__, __FUNCTION__, md5_apop_we, md5_apop_he);

  if (strcmp(md5_apop_he,makemd5(checkstring)) == 0) {
      trace(TRACE_MESSAGE,"%s,%s: user [%s] is validated using APOP",
			  __FILE__, __FUNCTION__, username);
	  /* get user idnr */
	  query_result = db_get_result(0, 1);	
